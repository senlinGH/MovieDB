...
//  MovieDB
//
//  Created by Lin Yi Sen on 2020/12/15.
...
class TopRatedViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        downloadTopRatedMovies()  // 下載TopRated的電影海報
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return topRatedArray.count
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = topRatedCollectionView.dequeueReusableCell(withReuseIdentifier: "posterCell", for: indexPath) as! PosterCollectionViewCell
        
        let posterPath = topRatedArray[indexPath.row]
    ...
    func forIpadAutoLayout() {
        ...
        let flowLayout = topRatedCollectionView.collectionViewLayout as? UICollectionViewFlowLayout
    ...
    // MARK: - 下載TopRated的電影海報
    func downloadTopRatedMovies() {
            
        let url = URL(string: "https://api.themoviedb.org/3/movie/top_rated?api_key=fa36146a9c9339288ef9538e4bb1abb6&language=zh-TW&page=\(pageNum)&region=TW")
        ...
        if let downloadURL = url {
            URLSession.shared.dataTask(with: downloadURL) { data, urlResponse, error in
                    ...
                    if self.pageNum <= totalPages {
                        ...
                        while item < downloadPosterPaths.results.count {
                            if let singlePoster = downloadPosterPaths.results[item].poster_path {
                                self.topRatedArray.append(singlePoster)
                                item += 1
                            }
                ...
                print("電影海報共\(self.topRatedArray.count)筆")
            }.resume()
        }
    }
