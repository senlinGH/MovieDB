...
class PopularViewController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        downloadPopularMovies()  // 下載Popular的電影海報
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let posterPath = popularArray[indexPath.row]
    ...
    func downloadPopularMovies() {
        ...
        let url = URL(string: "https://api.themoviedb.org/3/movie/popular?api_key=fa36146a9c9339288ef9538e4bb1abb6&language=zh-TW&page=\(pageNum)&region=TW")
        ...
        if let downloadURL = url {
            URLSession.shared.dataTask(with: downloadURL) { data, urlResponse, error in
                    ...
                    if self.pageNum <= totalPages {
                        ...
                        while item < downloadPosterPaths.results.count {
                            if let singlePoster = downloadPosterPaths.results[item].poster_path {
                                self.popularArray.append(singlePoster)
                                item += 1
                            }
                ...
                print("電影海報共\(self.popularArray.count)筆")
            }.resume()
        }
    }
